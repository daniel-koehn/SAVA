...
int *recswitch){
	...
	if (SEISMO & 16){
		/* stress */
                catseis(sectiontxx, section_fulldata, recswitch, ntr,ns);
		sprintf(seisfile,"%s%.4i_txx%s",SEIS_FILE,FFID,ext);
		fprintf(fp,"\t %s \n",seisfile);
		outseis(fp,fopen(seisfile,"w"),section_fulldata,recpos,recpos_loc,ntr,srcpos,nsrc,ns,SEIS_FORMAT,xpg,ypg,zpg);

		catseis(sectiontxy, section_fulldata, recswitch, ntr,ns);
		sprintf(seisfile,"%s%.4i_txy%s",SEIS_FILE,FFID,ext);
		fprintf(fp,"\t %s \n",seisfile);
		outseis(fp,fopen(seisfile,"w"),section_fulldata,recpos,recpos_loc,ntr,srcpos,nsrc,ns,SEIS_FORMAT,xpg,ypg,zpg);

		catseis(sectiontxz, section_fulldata, recswitch, ntr,ns);
		sprintf(seisfile,"%s%.4i_txz%s",SEIS_FILE,FFID,ext);
		fprintf(fp,"\t %s \n",seisfile);
		outseis(fp,fopen(seisfile,"w"),section_fulldata,recpos,recpos_loc,ntr,srcpos,nsrc,ns,SEIS_FORMAT,xpg,ypg,zpg);

		catseis(sectiontyy, section_fulldata, recswitch, ntr,ns);
		sprintf(seisfile,"%s%.4i_tyy%s",SEIS_FILE,FFID,ext);
		fprintf(fp,"\t %s \n",seisfile);
		outseis(fp,fopen(seisfile,"w"),section_fulldata,recpos,recpos_loc,ntr,srcpos,nsrc,ns,SEIS_FORMAT,xpg,ypg,zpg);

		catseis(sectiontyz, section_fulldata, recswitch, ntr,ns);
		sprintf(seisfile,"%s%.4i_tyz%s",SEIS_FILE,FFID,ext);
		fprintf(fp,"\t %s \n",seisfile);
		outseis(fp,fopen(seisfile,"w"),section_fulldata,recpos,recpos_loc,ntr,srcpos,nsrc,ns,SEIS_FORMAT,xpg,ypg,zpg);

		catseis(sectiontzz, section_fulldata, recswitch, ntr,ns);
	...
	if (SEISMO & 8){
		...
		catseis(sectionax, section_fulldata, recswitch, ntr,ns);
		sprintf(seisfile,"%s%.4i_ax%s",SEIS_FILE,FFID,ext);
		fprintf(fp,"\t %s \n",seisfile);
		outseis(fp,fopen(seisfile,"w"),section_fulldata,recpos,recpos_loc,ntr,srcpos,nsrc,ns,SEIS_FORMAT,xpg,ypg,zpg);

		catseis(sectionay, section_fulldata, recswitch, ntr,ns);
		sprintf(seisfile,"%s%.4i_ay%s",SEIS_FILE,FFID,ext);
		fprintf(fp,"\t %s \n",seisfile);
		outseis(fp,fopen(seisfile,"w"),section_fulldata,recpos,recpos_loc,ntr,srcpos,nsrc,ns,SEIS_FORMAT,xpg,ypg,zpg);

		catseis(sectionaz, section_fulldata, recswitch, ntr,ns);
	...
	if (SEISMO & 4){
		/* div and curl */
		catseis(sectiondiv, section_fulldata, recswitch, ntr,ns);
		sprintf(seisfile,"%s%.4i_div%s",SEIS_FILE,FFID,ext);
		fprintf(fp,"\t %s \n",seisfile);
		outseis(fp,fopen(seisfile,"w"),section_fulldata,recpos,recpos_loc,ntr,srcpos,nsrc,ns,SEIS_FORMAT,xpg,ypg,zpg);

		catseis(sectioncurlx, section_fulldata, recswitch, ntr,ns);
		sprintf(seisfile,"%s%.4i_curlx%s",SEIS_FILE,FFID,ext);
		fprintf(fp,"\t %s \n",seisfile);
		outseis(fp,fopen(seisfile,"w"),section_fulldata,recpos,recpos_loc,ntr,srcpos,nsrc,ns,SEIS_FORMAT,xpg,ypg,zpg);

		catseis(sectioncurly, section_fulldata, recswitch, ntr,ns);
		sprintf(seisfile,"%s%.4i_curly%s",SEIS_FILE,FFID,ext);
		fprintf(fp,"\t %s \n",seisfile);
		outseis(fp,fopen(seisfile,"w"),section_fulldata,recpos,recpos_loc,ntr,srcpos,nsrc,ns,SEIS_FORMAT,xpg,ypg,zpg);

		catseis(sectioncurlz, section_fulldata, recswitch, ntr,ns);
	...
	if (SEISMO & 2){
		/* pressure */
		catseis(sectionp, section_fulldata, recswitch, ntr,ns);
	...
	if (SEISMO & 1){
		/* particle velocity */ 
		catseis(sectionvx, section_fulldata, recswitch, ntr,ns);
		sprintf(seisfile,"%s%.4i_vx%s",SEIS_FILE,FFID,ext);
		fprintf(fp,"\t %s \n",seisfile);
		outseis(fp,fopen(seisfile,"w"),section_fulldata,recpos,recpos_loc,ntr,srcpos,nsrc,ns,SEIS_FORMAT,xpg,ypg,zpg);

		catseis(sectionvy, section_fulldata, recswitch, ntr,ns);
		sprintf(seisfile,"%s%.4i_vy%s",SEIS_FILE,FFID,ext);
		fprintf(fp,"\t %s \n",seisfile);
		outseis(fp,fopen(seisfile,"w"),section_fulldata,recpos,recpos_loc,ntr,srcpos,nsrc,ns,SEIS_FORMAT,xpg,ypg,zpg);

		catseis(sectionvz, section_fulldata, recswitch, ntr,ns);
