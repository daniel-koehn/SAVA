/*------------------------------------------------------------------------
 *  Parallel 3-D isotropic viscoelastic Finite Difference Modelling 
...
#include "fd3dps.h"	/* include file for P-S version */
#include "fd3diso.h"	/* include file for isotropic version */
#include "fd3del.h"	/* include file for elastic version */
#include "ISO_struct.h"	/* data structures for ISO forward modelling */
...
mat.u    = f3tensor(0,NX[0]+1,0,NX[1]+1,0,NX[2]+1);
...
mat.uipjk  	 = f3tensor(1,NX[0],1,NX[1],1,NX[2]);
mat.uijpk  	 = f3tensor(1,NX[0],1,NX[1],1,NX[2]);
mat.uijkp  	 = f3tensor(1,NX[0],1,NX[1],1,NX[2]);
...
/* create model grids */
if (READMOD)
	readmod(mat.rho,mat.pi,mat.u,mat.taus,mat.taup,mat.eta);
else
	model(mat.rho,mat.pi,mat.u,mat.taus,mat.taup,mat.eta,geom.xp,geom.yp,geom.zp);


/* check if the FD run will be stable and free of numerical dispersion */
checkfd(FP,mat.rho,mat.pi,mat.u,mat.taus,mat.taup,mat.eta,geom.x,geom.y,geom.z);
...
av_mat(mat.u,mat.uipjk,9,'h',geom.x,geom.xp,geom.y,geom.yp,geom.z,geom.zp);
av_mat(mat.u,mat.uijpk,7,'h',geom.x,geom.xp,geom.y,geom.yp,geom.z,geom.zp);
av_mat(mat.u,mat.uijkp,8,'h',geom.x,geom.xp,geom.y,geom.yp,geom.z,geom.zp);
...
fd_coeff(1, NX[0], 1, NX[1], 1, NX[2], geom.x, geom.y, geom.z, geom.xp, geom.yp, geom.zp,
	...
	mat.pi, mat.u, mat.uipjk, mat.uijpk, mat.uijkp, mat.rhoijpkp, mat.rhoipjkp, mat.rhoipjpk);
...
MPI_Recv_init(&mpi.rbuf_v_lef_to_rig[1][1],     NX[2]*NX[1],MPI_FLOAT,INDEX[1],TAG1,COMM_CART,&mpi.request_v[0]);
MPI_Recv_init(&mpi.rbuf_v_rig_to_lef[1][1][1],2*NX[2]*NX[1],MPI_FLOAT,INDEX[0],TAG2,COMM_CART,&mpi.request_v[1]);
MPI_Recv_init(&mpi.rbuf_v_bac_to_fro[1][1],     NX[2]*NX[0],MPI_FLOAT,INDEX[3],TAG3,COMM_CART,&mpi.request_v[2]);
MPI_Recv_init(&mpi.rbuf_v_fro_to_bac[1][1][1],2*NX[2]*NX[0],MPI_FLOAT,INDEX[2],TAG4,COMM_CART,&mpi.request_v[3]);
MPI_Recv_init(&mpi.rbuf_v_top_to_bot[1][1],     NX[1]*NX[0],MPI_FLOAT,INDEX[5],TAG5,COMM_CART,&mpi.request_v[4]);
MPI_Recv_init(&mpi.rbuf_v_bot_to_top[1][1][1],2*NX[1]*NX[0],MPI_FLOAT,INDEX[4],TAG6,COMM_CART,&mpi.request_v[5]);
MPI_Send_init(&mpi.sbuf_v_lef_to_rig[1][1],     NX[2]*NX[1],MPI_FLOAT,INDEX[0],TAG1,COMM_CART,&mpi.request_v[6]);
MPI_Send_init(&mpi.sbuf_v_rig_to_lef[1][1][1],2*NX[2]*NX[1],MPI_FLOAT,INDEX[1],TAG2,COMM_CART,&mpi.request_v[7]);
MPI_Send_init(&mpi.sbuf_v_bac_to_fro[1][1],     NX[2]*NX[0],MPI_FLOAT,INDEX[2],TAG3,COMM_CART,&mpi.request_v[8]);
MPI_Send_init(&mpi.sbuf_v_fro_to_bac[1][1][1],2*NX[2]*NX[0],MPI_FLOAT,INDEX[3],TAG4,COMM_CART,&mpi.request_v[9]);
MPI_Send_init(&mpi.sbuf_v_top_to_bot[1][1],     NX[1]*NX[0],MPI_FLOAT,INDEX[4],TAG5,COMM_CART,&mpi.request_v[10]);
MPI_Send_init(&mpi.sbuf_v_bot_to_top[1][1][1],2*NX[1]*NX[0],MPI_FLOAT,INDEX[5],TAG6,COMM_CART,&mpi.request_v[11]);
...
MPI_Recv_init(&mpi.rbuf_s_lef_to_rig[1][1][1],2*NX[2]*NX[1],MPI_FLOAT,INDEX[1],TAG7, COMM_CART,&mpi.request_s[0]);
MPI_Recv_init(&mpi.rbuf_s_rig_to_lef[1][1],     NX[2]*NX[1],MPI_FLOAT,INDEX[0],TAG8, COMM_CART,&mpi.request_s[1]);
MPI_Recv_init(&mpi.rbuf_s_bac_to_fro[1][1][1],2*NX[2]*NX[0],MPI_FLOAT,INDEX[3],TAG9, COMM_CART,&mpi.request_s[2]);
MPI_Recv_init(&mpi.rbuf_s_fro_to_bac[1][1],     NX[2]*NX[0],MPI_FLOAT,INDEX[2],TAG10,COMM_CART,&mpi.request_s[3]);
MPI_Recv_init(&mpi.rbuf_s_top_to_bot[1][1][1],2*NX[1]*NX[0],MPI_FLOAT,INDEX[5],TAG11,COMM_CART,&mpi.request_s[4]);
MPI_Recv_init(&mpi.rbuf_s_bot_to_top[1][1],     NX[1]*NX[0],MPI_FLOAT,INDEX[4],TAG12,COMM_CART,&mpi.request_s[5]);
MPI_Send_init(&mpi.sbuf_s_lef_to_rig[1][1][1],2*NX[2]*NX[1],MPI_FLOAT,INDEX[0],TAG7, COMM_CART,&mpi.request_s[6]);
MPI_Send_init(&mpi.sbuf_s_rig_to_lef[1][1],     NX[2]*NX[1],MPI_FLOAT,INDEX[1],TAG8, COMM_CART,&mpi.request_s[7]);
MPI_Send_init(&mpi.sbuf_s_bac_to_fro[1][1][1],2*NX[2]*NX[0],MPI_FLOAT,INDEX[2],TAG9, COMM_CART,&mpi.request_s[8]);
MPI_Send_init(&mpi.sbuf_s_fro_to_bac[1][1],     NX[2]*NX[0],MPI_FLOAT,INDEX[3],TAG10,COMM_CART,&mpi.request_s[9]);
MPI_Send_init(&mpi.sbuf_s_top_to_bot[1][1][1],2*NX[1]*NX[0],MPI_FLOAT,INDEX[4],TAG11,COMM_CART,&mpi.request_s[10]);
MPI_Send_init(&mpi.sbuf_s_bot_to_top[1][1],     NX[1]*NX[0],MPI_FLOAT,INDEX[5],TAG12,COMM_CART,&mpi.request_s[11]);
...
for (nt=1;nt<=NT;nt++){
	...
	if (L)  /* isotropic viscoelastic simulation */
		times.time_update[4] = update_s_ve(1, NX[0], 1, NX[1], 1, NX[2], wave.v, wave.t, wave.r, wave.w, 
					mat.pi, mat.u, mat.uipjk, mat.uijpk, mat.uijkp,
					mat.taup, mat.taus, mat.tausipjk, mat.tausijpk, mat.tausijkp, mat.eta, 
					pmls.absorb_coeff, 
					geom.dx, geom.dxp, geom.dy, geom.dyp, geom.dz, geom.dzp, infoout);
	else  /* isotropic elastic simulation (no visco-elasticity) */
		times.time_update[4] = update_s_el(1, NX[0], 1, NX[1], 1, NX[2], wave.v, wave.t, wave.w, 
					mat.pi, mat.u, mat.uipjk, mat.uijpk, mat.uijkp,
					pmls.absorb_coeff,
					geom.dx, geom.dxp, geom.dy, geom.dyp, geom.dz, geom.dzp, infoout);

	if (PML) times.time_update[5] = pml_update_s(wave.v, wave.t, mat.pi, mat.u, mat.uipjk, mat.uijpk, mat.uijkp, 
					pmls.pmlle, pmls.pmlri, pmls.pmlba, pmls.pmlfr, pmls.pmlto, pmls.pmlbo, 
...
free_f3tensor(mat.u,   0,NX[0]+1,0,NX[1]+1,0,NX[2]+1);
free_f3tensor(mat.taus,0,NX[0]+1,0,NX[1]+1,0,NX[2]+1);
free_f3tensor(mat.taup,0,NX[0]+1,0,NX[1]+1,0,NX[2]+1);

free_f3tensor(mat.uipjk,   1,NX[0],1,NX[1],1,NX[2]);
free_f3tensor(mat.uijpk,   1,NX[0],1,NX[1],1,NX[2]);
free_f3tensor(mat.uijkp,   1,NX[0],1,NX[1],1,NX[2]);
