...
void write_par(FILE *fp){
	...
	if (READMOD){
		...
		if (L){
			...
			fprintf(fp,"\t Q for shear waves:\n\t %s.qs\n",MFILE);
		}
	}
	else{
		fprintf(fp," use model defined in source code (model_ps.c) \n");
	}
	...
	if (L){
		...
		for (l=1;l<=L;l++)
			fprintf(fp,"\t%f",FL[l]);
		fprintf(fp," Hz\n");
		fprintf(fp," Global value for tau is : %f\n (is ignored if Qp and Qs are read from model files !)",TAU);
	}
	...
	if (SNAP & 31){
		...
		if (SNAP & 16)
			fprintf(fp,"\t xx-, xy-, xz-, yy-, yz- and zz-component of stress\n");
		if (SNAP & 8)
			fprintf(fp,"\t x-, y- and z-component of particle acceleration\n");
		if (SNAP & 4){
			fprintf(fp,"\t x-, y- and z-component of curl of the wavefield\n");
