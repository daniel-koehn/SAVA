...
void checkfd(FILE *fp, float *** rho, float *** pi, float *** u, float *** taus, float *** taup, float *eta, float *x, float *y, float *z){
	...
	float		vsmax_loc=0.0, vsmin_loc=1e9;
	...
	for (i=1;i<=NX[0];i++){
		for (j=1;j<=NX[1];j++){
			for (k=1;k<=NX[2];k++){
				...
				/* maximum phase velocity of S-waves at infinite frequency */
				vsmax = sqrt((u[i][j][k]/rho[i][j][k])*(1.0+L*taus[i][j][k]));

				/* minimum phase velocity of S-waves at center frequency of the source */
				vsmin = sqrt((u[i][j][k]/rho[i][j][k])*(1.0+sum*taus[i][j][k]));
				...
				vmax = max(vpmax,vsmax);
				if (vsmin > 0.0)
					vmin = min(vpmin,vsmin);
				else
					vmin = vpmin;
				dhmax = max(dx,dy);
				dhmax = max(dhmax,dz);
				...
				vsmax_loc = max(vsmax_loc,vsmax);
				vsmin_loc = min(vsmin_loc,vsmin);
	...
	vmax_loc = max(vpmax_loc,vsmax_loc);
	if (vsmin_loc > 0.0)
		vmin_loc = min(vpmin_loc,vsmin_loc);
	else
		vmin_loc = vpmin_loc;
	...
	fprintf(fp," Minimum and maximum P-wave velocity within subvolume (MYID=%d): \n",MYID);
	fprintf(fp," vP_max(f=inf)=%e m/s\t vP_min(f=fc)=%e m/s\n\n", vpmax_loc,vpmin_loc);

	fprintf(fp," Minimum and maximum S-wave velocity within subvolume (MYID=%d): \n",MYID);
	fprintf(fp," vS_max(f=inf)=%e m/s\t vS_min(f=fc)=%e m/s\n\n", vsmax_loc,vsmin_loc);
	...
	if (!(MYID)){
		...
		fprintf(fp," per minimum wavelength (of S-waves) should be 10 (better more).\n");
