		...
		for (i=1;i<=NX[0];i++){
			for (j=1;j<=NX[1];j++){
				for (k=1;k<=FWTO;k++){
					...
					/* Apply PML BC */
					divv = (pmlto[l].xp1*Pv_z_t[i][j][k].z + pmlto[l].xp2*vz_z);
					d    = (pi[i][j][k]-2.0*u[i][j][k])*divv;
					t[i][j][k].xx += d;
					t[i][j][k].yy += d;
					t[i][j][k].zz += pi[i][j][k]*divv;
					t[i][j][k].yz += uipjk[i][j][k]*(pmlto[l].x1*Pv_z_t[i][j][k].y + pmlto[l].x2*vy_z);
					t[i][j][k].xz += uijpk[i][j][k]*(pmlto[l].x1*Pv_z_t[i][j][k].x + pmlto[l].x2*vx_z);
		...
		for (i=1;i<=NX[0];i++){
			for (j=1;j<=NX[1];j++){
				for (k=NX[2]-FWBO+1;k<=NX[2];k++){
					...
					/* Apply PML BC */
					divv = (pmlbo[l].xp1*Pv_z_b[i][j][k].z + pmlbo[l].xp2*vz_z);
					d    = (pi[i][j][k]-2.0*u[i][j][k])*divv;
					t[i][j][k].xx += d;
					t[i][j][k].yy += d;
					t[i][j][k].zz += pi[i][j][k]*divv;
					t[i][j][k].yz += uipjk[i][j][k]*(pmlbo[l].x1*Pv_z_b[i][j][k].y + pmlbo[l].x2*vy_z);
					t[i][j][k].xz += uijpk[i][j][k]*(pmlbo[l].x1*Pv_z_b[i][j][k].x + pmlbo[l].x2*vx_z);
		...
		for (i=1;i<=NX[0];i++){
			for (j=1;j<=FWBA;j++){
				...
				for (k=1;k<=NX[2];k++){
					...
					/* Apply PML BC */
					divv = (pmlba[l].xp1*Pv_y_b[i][j][k].y + pmlba[l].xp2*vy_y);
					d    = (pi[i][j][k]-2.0*u[i][j][k])*divv;
					t[i][j][k].xx += d;
					t[i][j][k].yy += pi[i][j][k]*divv;
					t[i][j][k].zz += d;
					t[i][j][k].yz += uipjk[i][j][k]*(pmlba[l].x1*Pv_y_b[i][j][k].z + pmlba[l].x2*vz_y);
					t[i][j][k].xy += uijkp[i][j][k]*(pmlba[l].x1*Pv_y_b[i][j][k].x + pmlba[l].x2*vx_y);
		...
		for (i=1;i<=NX[0];i++){
			for (j=NX[1]-FWFR+1;j<=NX[1];j++){
				...
				for (k=1;k<=NX[2];k++){
					...
					/* Apply PML BC */
					divv = (pmlfr[l].xp1*Pv_y_f[i][j][k].y + pmlfr[l].xp2*vy_y);
					d    = (pi[i][j][k]-2.0*u[i][j][k])*divv;
					t[i][j][k].xx += d;
					t[i][j][k].yy += pi[i][j][k]*divv;
					t[i][j][k].zz += d;
					t[i][j][k].yz += uipjk[i][j][k]*(pmlfr[l].x1*Pv_y_f[i][j][k].z + pmlfr[l].x2*vz_y);
					t[i][j][k].xy += uijkp[i][j][k]*(pmlfr[l].x1*Pv_y_f[i][j][k].x + pmlfr[l].x2*vx_y);
		...
		for (i=1;i<=FWLE;i++){
			...
			for (j=1;j<=NX[1];j++){
				for (k=1;k<=NX[2];k++){
					...
					/* Apply PML BC */
					divv = (pmlle[l].xp1*Pv_x_l[i][j][k].x + pmlle[l].xp2*vx_x);
					d    = (pi[i][j][k]-2.0*u[i][j][k])*divv;
					t[i][j][k].xx += pi[i][j][k]*divv;
					t[i][j][k].yy += d;
					t[i][j][k].zz += d;
					t[i][j][k].xz += uijpk[i][j][k]*(pmlle[l].x1*Pv_x_l[i][j][k].z + pmlle[l].x2*vz_x);
					t[i][j][k].xy += uijkp[i][j][k]*(pmlle[l].x1*Pv_x_l[i][j][k].y + pmlle[l].x2*vy_x);
		...
		for (i=NX[0]-FWRI+1;i<=NX[0];i++){
			...
			for (j=1;j<=NX[1];j++){
				for (k=1;k<=NX[2];k++){
					...
					/* Apply PML BC */
					divv = (pmlri[l].xp1*Pv_x_r[i][j][k].x + pmlri[l].xp2*vx_x);
					d    = (pi[i][j][k]-2.0*u[i][j][k])*divv;
					t[i][j][k].xx += pi[i][j][k]*divv;
					t[i][j][k].yy += d;
					t[i][j][k].zz += d;
					t[i][j][k].xz += uijpk[i][j][k]*(pmlri[l].x1*Pv_x_r[i][j][k].z + pmlri[l].x2*vz_x);
					t[i][j][k].xy += uijkp[i][j][k]*(pmlri[l].x1*Pv_x_r[i][j][k].y + pmlri[l].x2*vy_x);
